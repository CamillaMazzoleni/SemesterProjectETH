...
def visualize_cubes_masked(vertices, faces, color, assign_matrix, save_path, bias, pred, names):
    ...
    for k in range(batch_size):
        ...
        print(f"verticces shape : {vertices.shape}")
        vertices_masked = vertices[k,:,:].reshape(num_cuboids,8,3)[mask,:,:].reshape(-1,3)
        print(f"vertices masked shape : {vertices_masked.shape}")

        print(f"faces shape : {faces.shape}")
        faces_masked = faces[k,:mask.sum()*24,:]
        print(f"faces_masked shape : {faces_masked.shape}")
...
def visualize_segmentation(pc, color, assign_matrix, save_path, bias, names):
    ...
    assign_matrix = assign_matrix.cpu().numpy()
    save_path = 'PretrainModels/chair/infer/test/'
    print(assign_matrix.shape)
    for k in range(batch_size):
        if names is not None:
            filename = save_path + 'mask/' + names[k] + '_segment.ply'
        else:
            filename = save_path + 'mask/' + str(bias + k) + '_segment.ply'
        color_segment = color[np.argmax(assign_matrix[k,:,:],1),:]
        export_pc(pc[k,:,:], color_segment, filename)

        

def save_cubes_json(scale, rotate, trans, exists, save_path, names, color):
    ...
    mask = exists[k,:]>0.5
    #color = (num_cuboids, 3)
    #filter the colors with the mask
    color[mask,:][:,np.newaxis,:].repeat(24,axis = 1).reshape(-1,3)
    ...
    for el in range(batch_size):
        ...
        for k in range(num_el):
            if exists[el,k] > 0.5:
                ...
                #give the respective color to the component
                component['color'] = color[k,:].detach().cpu().numpy().tolist()
        ...
        json.dump(res, open(save_path + 'mask/' + names[el] + '.json', 'w'))

    



def visualize_points_masked(pc, assign_matrix, color, save_path, bias, pred, names, threshold=0.5):
    batch_size = assign_matrix.shape[0]
    num_points = pc.shape[1]

    print(f"Initial pointcloud shape {pc.shape}")
    
    pc = pc.cpu().detach().numpy()
    assign_matrix = assign_matrix.cpu().detach().numpy()
    
    for k in range(batch_size):
        if names is not None:
            filename = save_path + names[k] + '_points_masked' + pred + '.ply'
        else:
            filename = save_path + str(bias + k) + '_points_masked' + pred + '.ply'
        
        # Compute the mask based on the sum of assignments for each point
        mask = (assign_matrix[k, :, :].max(axis=1) > threshold)
        
        print(f"Mask shape: {mask.shape}")
        print(f"Point cloud shape: {pc.shape}")
        
        # Apply the mask to the points and colors
        points_masked = pc[k, mask, :]
        colors_masked = color[np.argmax(assign_matrix[k, mask, :], axis=1), :]
        
        print(f"Masked points shape: {points_masked.shape}")
        print(f"Masked colors shape: {colors_masked.shape}")
        
        export_pc(points_masked, colors_masked, filename)
